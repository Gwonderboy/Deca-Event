  ...
  try {
    const response = await transport.sendMail({
      ...
      html: `<div width="50%" style="text-align: center; padding: 25px; border-radius: 5px; border: 2px solid #27AE60;"><h1>Welcome to Deca Events</h1>
            ...
            <a href="${process.env.APP_BASE_URL}/verify?token=${token}" style="text-align: center; padding: 10px; border-radius: 10px; background: #27AE60; text-decoration: none; color: white;">Verify Account</a></div>`,
    });
  } catch (err: any) {

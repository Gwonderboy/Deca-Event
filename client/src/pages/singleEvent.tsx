...
function SingleEvent() {
  ...
  const fetchEventComments = async () => {
    try {
      ...
      const eve: any = localStorage.getItem("event");
      const ever = JSON.parse(eve);
      console.log(ever);
    } catch (error: any) {
      if (error.response) {
        return showErrorToast(error.response.data.message);
  ...
  const fetchUpcomingEvents = async () => {
    try {
      ...
      return setUpcomingEvents(response.data);
    } catch (error: any) {
      console.log(error);
    }
  };
        ...
        <div className="ml-16 mr-16 mt-2">
          ...
          <div>
            <SingleEventBody
              ...
              organizerInfo={`${organizer.name_of_organizer}: ${organizer.email_of_organiser}`}
              organizerImage={organizer.image_of_organizer}
            />
          </div>
          {/* comment section */}
          ...
          <div className="py-3 bg-gray-100 p-[20px] h-[300px] overflow-y-scroll">
            {fetchedComments &&
              fetchedComments.map((comment: any, index: any) => (
                <div key={index}>
                  <div className="flex">
                    <img
                      src={comment.picture}
            ...
            <div className="text-gray-900 text-base font-medium font-Inter leading-snug tracking-tight">
              ...
              <div className="flex gap-3 overflow-x-scroll">
                {upcomingEvents.length !== 0
                  ? upcomingEvents?.map((events: any, index: any) => (
                      <div key={index}>
                        <Cards

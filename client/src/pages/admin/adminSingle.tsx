...
import { getSingeEvent, addEventComments, getEventComments, userDeleteEvent, userLikesAnEvent, userDislikesAnEvent, adminBlocksEvent, adminUnblocksEvent, getEventReport } from "../../axiosSettings/events/eventAxios";
        ...
        <Modal onClose={() => setReportModal(false)}>
         <div className="text-center w-[90%] h-[500px] mt-[20px] mb-[20px] overflow-y-scroll">
          ...
          <div className="text-left bg-gray-100 w-[100%]">
            {getReports ? (getReports?.map((repo:any, index:any)=>(
              <>
              <div key={index} className="border p-[10px] h-[200px] overflow-y-scroll">
                <p><span className="text-[#27AE60]">Name of Reporter:</span> {repo.owner_name}</p>
                <p><span className="text-[#27AE60]">Report:</span> {repo.report}</p>
              </div>
              <br />
              </>
            ))):(<p>No Reports</p>)}

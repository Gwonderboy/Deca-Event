...
export const CreateEventPage = () => {
  ...
  //handling events time selection
  ...
  const handleSubmit = async () => {
    try {
      ...
      navigate(`/single-event-organizer/${eventId}`)
      return setLoading(false);
    } catch (error: any) {
      if (error.response) {
  ...
  const buttons2:any = [
    {
      label: `Accept and Proceed to Create Event`,
  ...
  return (
    <>
      ...
      <div className="flex justify-center">
        ...
        {showModal2 && (
        <Modal onClose={() => rejectPrivacy()} buttons={buttons2}>
          <div className="text-center w-[90%] h-[400px] mt-[20px] mb-[20px] overflow-y-scroll">
